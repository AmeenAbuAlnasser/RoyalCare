  ...
  return (
    <SuperAdminActionMenu
      ...
      items={[
        ...
        { icon: "verify", label: dictionary.actions.forcePasswordChange },
        { icon: "read", label: dictionary.actions.sendWelcomeEmail },
                  ...
                  <tr>
                    {[
                      ...
                      >
                        {
                          dictionary.table[
                            column as keyof typeof dictionary.table
                          ]
                        }
                      </th>
                    ))}
                ...
                <tbody>
                  {userRows.map((row) => (
                    <tr className="border-t border-[#E5E7EB]" key={row.id}>
                      <td className="whitespace-nowrap px-4 py-4 font-semibold text-[#0B2D5C]">
                        {dictionary.names[row.nameKey]}
                      </td>
                      <td className="whitespace-nowrap px-4 py-4 text-[#526176]">
                        {row.email}
                      </td>
                      <td className="whitespace-nowrap px-4 py-4 text-[#526176]">
                        {dictionary.roles[row.role]}
                      </td>
                      <td className="whitespace-nowrap px-4 py-4 text-[#526176]">
                        {dictionary.departments[row.department]}
                      </td>
                      ...
                      <td className="whitespace-nowrap px-4 py-4 text-[#526176]">
                        {row.lastLogin
                          ? formatDate(row.lastLogin, locale)
                ...
                >
                  ...
                  <div className="mt-4 grid min-w-0 grid-cols-1 gap-3">
                    <DetailPair
                      ...
                      value={dictionary.roles[row.role]}
                    />
                    <DetailPair
                      label={dictionary.table.department}
                      value={dictionary.departments[row.department]}
                    />
                    <DetailPair
                      label={dictionary.table.lastLogin}
                      value={
                        row.lastLogin
                          ? formatDate(row.lastLogin, locale)
                          : dictionary.values.neverLoggedIn
                    ...
                    />
                    <DetailPair
                      label={dictionary.table.createdDate}
                      value={formatDate(row.createdDate, locale)}
                    />
                  </div>
                  <div className="mt-4">
                    <ActionMenu
