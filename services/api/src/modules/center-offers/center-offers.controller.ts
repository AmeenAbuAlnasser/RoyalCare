...
async function saveOfferImage(
  ...
  try {
    ...
    const filename = `offer-${Date.now()}-${randomUUID()}${outExt}`;
    await writeFile(join(uploadDir, filename), outBuffer);
    return { url: `/uploads/offers/${filename}` };
  } catch (error) {
    console.error('[tenant-offers] image write error:', error);
    throw new BadRequestException({
...
export class PublicCenterOffersController {
  constructor(private readonly service: CenterOffersService) {}
...
@Controller('tenant/offers')
export class TenantCenterOffersController {
  constructor(
    private readonly centerAuthService: CenterAuthService,
    private readonly service: CenterOffersService,
  ) {}
  ...
  async upload(
    ...
    return saveOfferImage(file);
  }

  @Post()
  async create(@Req() request: Request, @Body() body: OfferPayload) {
    const session = await this.getSession(request);
